...
namespace RentWise.Agent.Controllers
{
    ...
    public class DashboardController : Controller
    {
            ...
            if (order != null)
            {
                ...
                if (LkpStatus == 2)
                {
                  Message = "Your reservation has been accepted";
                    ...
                    SharedFunctions.SendPushNotification(ToUsersDetails.OneSignalId, "Your reservation has been accepted", "Your order has been accepted, you can now make payment");
                }   else
                {
                    ...
                    SharedFunctions.SendPushNotification(ToUsersDetails.OneSignalId, "Your reservation has been rejected", "Your order has been rejected, look for another listing");
                }
        ...
        public async Task<IActionResult> PaymentReceived(int Id)
        {
            ...
            if (order != null)
            {
                string Message = "Payment received by Owner, Contact Owner on " + agent.PhoneNumber;
                ChatModel chat = new()
                {
                    FromUserId = order.AgentId,
                    ToUserId = order.UserId,
